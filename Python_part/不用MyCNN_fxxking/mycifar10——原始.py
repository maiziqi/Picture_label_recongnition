...
def inference(images,keep_prob):									#keep_prob是有效概率
	...
	with tf.variable_scope('myconv2') as scope:
		kernel=_variable_with_weight_decay('weights',shape=[5,5,48,128],
										stddev=1e-1,wd=0.0)
		conv=tf.nn.conv2d(mynorm1,kernel,[1,1,1,1],padding='SAME')
		biases=_variable_on_cpu('biaes',[128],tf.constant_initializer(0.0))
		bias=tf.nn.bias_add(conv,biases)
	...
	with tf.variable_scope('myconv3') as scope:
		...
		biases=_variable_on_cpu('biaes',[192],tf.constant_initializer(0.0))
		bias=tf.nn.bias_add(conv,biases)
...
#	mynorm4=tf.nn.lrn(mypool4,4,bias=1.0,alpha=0.001/9.0,beta=0.75,name='mynorm4')
	...
	with tf.variable_scope('myconv5') as scope:
		kernel=_variable_with_weight_decay('weights',shape=[3,3,192,128],
										stddev=1e-1,wd=0.0)
	...
	with tf.variable_scope('local1') as scope:
		...
		weights=_variable_with_weight_decay('weights',shape=[dim,1024],					#384
